...
Dirty.prototype.seek = function(fn, index) {
  this._index = index || this._index || 0;
  for (var i = this._index, l = this._docs.length; i < l; i ++) {
    if (fn(this._docs[i])) return this._docs[i];
  }
  return null;
};
